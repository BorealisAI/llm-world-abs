...
def load(ckpt_dir, adapter_dir=None):
    ...
    model = AutoModelForCausalLM.from_pretrained(ckpt_dir, low_cpu_mem_usage=True, load_in_8bit=True)#torch_dtype=torch.float16)
    ...
    for i, data in enumerate(dataset):
        ...
        if args.preserve_ops:
            for box in env.state.boxes:
                box.max_items = 1000 # ugly but necessary
