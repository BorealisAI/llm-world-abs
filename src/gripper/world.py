...
class Environment:
    ...
    def get_legal_actions(self, prev):
        ...
        for obj in self.agent.objects:
            if (prev is None or prev.atype != 'grab' or prev.obj.name != obj.name) and not self.state.boxes[self.agent.location].full():
                acts_ok.append(
                    Action('put', obj=obj)
                )
        ...
        if self.agent.location > 0:
            if prev is None or (prev.atype != 'move' or prev.direct != 'right') or len(acts_ok) == 0:
                acts_ok.append(
                    Action('move', direct='left', step=1)
                )
        
        if self.agent.location < self.state.num_boxes - 1:
            if prev is None or prev.atype != 'move' or prev.direct != 'left' or len(acts_ok) == 0:
                acts_ok.append(
    ...
    def get_valuable_actions(self, ops_hist):
        acts_ok = []
        # get legal move 
        prev = ops_hist[-1]
        for obj in self.state.boxes[self.agent.location]:
