...
def pad_lmr(llm_repr_options, pred_data, task):
    '''
    llm_repr_options: List[{str: repr}]
    ...
    llm_repr_dummy = torch.zeros_like(
        [list(x.values())[0] for x in llm_repr_options if len(x.keys()) > 0][0]
    )
    label_cand_lmr = []
    for pd, lmr_opt in zip(pred_data, llm_repr_options):
        label_cand_lmr.append(
            torch.stack(
                [
                    lmr_opt[pd[task][f'cand{i}']]
                    if task in pd.keys() and f'cand{i}' in pd[task]
                    else llm_repr_dummy
...
def collate_fn(features, state_type, debug=False):
    '''
    {pred_key: {"cand{i}": torch.tensor}}
    output: {pred_key: {"options": [bsz, ncand, dim], "label": int}}
    '''
    task2sub = _TASKS[state_type]
    predicate_data = [x['predicate_data'][state_type] for x in features]  # {type: {pk: {"cand{i}": text, "label": int}}}

    llm_repr_ents = [x['ents_lmr'] for x in features]
    ...
    for task, subs in task2sub.items():
        ...
        if subs['task_type'] == 'rank':
            # bsz x ncand x dim
            tmp = pad_lmr(llm_repr_ents, predicate_data, task)
            if tmp is None:
        ...
        def run_epoch(split):
            ...
            loader = DataLoader(data,
                                ...
                                collate_fn=lambda x: collate_fn(
                                    x, self.state_type, self.label_repr
                                    )
                                )
            losses = []
            ...
            for it, pd in pbar:
                ...
                if is_train:
                    ...
                    pbar.set_description(
                        f"epoch {epoch+1}: train loss {mean_loss:.5f}; lr {lr:.2e}; Acc [{prt_acc}]"
                    )
            ...
            if not is_train:
                ...
                if prt:
                    prt_acc = self.metric.log()
                    logger.info(f"test loss {test_loss:.5f}; Acc [{prt_acc}]")
