...
export LOG_DIR="$HOME/llm_world_abs/cook_probe_logs"
export LLM="" # LLM name, e.g. llama3
export LLM_MODEL="" # {Meta-Llama-3-8B-Instruct, Mistral-7B-Instruct-v0.2, Llama-2-13b-chat-hf, Llama-2-7b-chat-hf, Phi-3-medium-4k-instruct, Phi-3-mini-4k-instruct}
export ADAPT_DIR="None" # path to fine-tuned LLM or None(for ICL)
export ICL_DATA="$HOME/cook_dataset/val.pkl" # 
export PROMPT_METHOD='' #{'icl', 'inst-ft'} 'ft' for pythia
export DIM=8600
...
export TRAIN_DATA="$HOME/cook_dataset/train.pkl"
export TEST_DATA="$HOME/cook_dataset/test.pkl"

mkdir $LOG_DIR
mkdir "${LOG_DIR}/${LLM}"
# box_name, box_content, agent_has_t, aloc, subgoal, relative_pos
export PROBE_DATA_TRAIN="$HOME/cook_probe_dataset/probe_${LLM}_${PROMPT_METHOD}/train/"
export PROBE_DATA_TEST="$HOME/cook_probe_dataset/probe_${LLM}_${PROMPT_METHOD}/test/"
export PROBE_DATA_LMR="$HOME/cook_probe_dataset/probe_${LLM}_${PROMPT_METHOD}/label_embed.pkl"
# MAKE DATA
python3 -u $HOME/llm_world_abs/src/cook/probe/make_probe_data.py \
...
python3 -u $HOME/llm_world_abs/src/cook/probe/make_label_lmr.py \
...
for state_type in "raw" "world" "legal" "policy" "ndest"; do
    export out_model_path="$HOME/llm_world_abs/src/cook/models_probe/${LLM}_${state_type}_probe_l2_dim${DIM}_last6/"
    echo "${LOG_DIR}/${LLM}_${state_type}_${DIM}.log"
    python3 -u $HOME/llm_world_abs/src/cook/probe/train.py \
...
export PROBE_DATA_TRAIN="$HOME/cook_probe_dataset/probe_final_${LLM}_${PROMPT_METHOD}/train/"
export PROBE_DATA_TEST="$HOME/cook_probe_dataset/probe_final_${LLM}_${PROMPT_METHOD}/test/"
...
for state_type in "world_dream"; do
    export out_model_path="$HOME/llm_world_abs/src/cook/models_final_probe/llm=${LLM}_${state_type}_probe_l2_dim${DIM}_last6/"
    python3 -u $HOME/llm_world_abs/src/cook/probe/train.py \
        --train_data_path $PROBE_DATA_TRAIN \
        --val_data_path $PROBE_DATA_TEST \
        --label_repr_path $PROBE_DATA_LMR\
...
# nearby
export PROBE_DATA_TRAIN="$HOME/cook_probe_dataset/probe_dyna_${LLM}_${PROMPT_METHOD}/train/"
export PROBE_DATA_TEST="$HOME/cook_probe_dataset/probe_dyna_${LLM}_${PROMPT_METHOD}/test/"

# make nearby data
python3 -u $HOME/probe_lm_world/cook/probe/make_probe_data_dyna.py \
        --dataset_path $TRAIN_DATA\
        --output_dataset_path $PROBE_DATA_TRAIN\
        --ckpt_dir $LLM_MODEL --prompt_method $PROMPT_METHOD  --icl_dataset_path $ICL_DATA\
...
# obj_ctrl
export PROBE_DATA_TRAIN="$HOME/cook_probe_dataset/probe_objCtrl_${LLM}_${PROMPT_METHOD}/train/"
export PROBE_DATA_TEST="$HOME/cook_probe_dataset/probe_objCtrl_${LLM}_${PROMPT_METHOD}/test/"
