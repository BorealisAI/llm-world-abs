...
from inference import LLamaPrompter
...
def get_args():
    ...
    parser.add_argument("--dataset_script_path", type=str, default="")
...
def create_datasets(tokenizer, formatter, args):
    ...
    train_dataset = train_dataset.train_test_split(test_size=0.005, seed=args.seed)
    valid_data = train_dataset["test"]
    train_data = train_dataset["train"]

    train_dataset = train_data.map(prepare_sample_text)
    valid_dataset = valid_data.map(prepare_sample_text)
