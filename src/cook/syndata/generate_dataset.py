...
_MODIFIERS = {
    "cook": ["raw", "fried", "roasted", "grilled"],
    "size": ["uncut", "chopped", "sliced", "diced"]
}

_SPLITS_PROP = {"train": 0.5, "dev": 0.1, "test": 0.4}

_CONTAINER_NAMES = ['kitchen', 'pantry', 'livingroom', 'bathroom', 'bedroom', 'backyard', 'garden']
...
def syn_data(args):
    ...
    for init_s, curr_s, final_s, init_loc, curr_loc, final_loc, ops, noi, objs in sampled_sequences:
        ...
        if len(ops_rollout) <= 3:
            print("NOI", noi)
            print('INIT')
            for box in init_s.boxes:
                print([x.nickname() for x in box])
            print('CURR')
            for box in curr_s.boxes:
                print([x.nickname() for x in box])
            print('FINAL')
            for box in final_s.boxes:
                print([x.nickname() for x in box])
            print("Roll IN")
...
def syn_data_augment(args, debug=False):
    ...
    for init_s, curr_s, final_s, init_loc, curr_loc, final_loc, ops, noi, objs in sampled_sequences:
        ...
        if debug and len(ops_rollout) <= 3:
            print("NOI", noi)
            print('INIT')
            for box in init_s.boxes:
                print([x.nickname() for x in box])
...
def syn_data_mix(args):

    objects_set = load_objects_from_csv(args.object_vocabulary_file)
    rare_objects_set = load_objects_from_csv(args.object_vocabulary_file_2)

    sampled_sequences = []
    for _ in tqdm.tqdm(range(args.num_data)):
        num_operations_init = random.choice(list(range(args.num_operations_init+1)))
        num_operations = random.choice(list(range(
                                (0 if num_operations_init > 0 else 1), 
                                args.num_operations+1))
                            )
        ...
        if random.uniform(0, 1) < 0.5:
            ...
            args.modifier = "both"
            entities = create_objects(args, rare_objects_set)
            osn = 'rare'
        ...
        sampled_sequences.append(
            (init_state_copy, curr_state, final_state, init_loc, curr_loc, final_loc, operation_sequence, env.gallery, meta_info)
        )  

    dataset = []
    existing_states_pairs = []

    for init_s, curr_s, final_s, init_loc, curr_loc, final_loc, ops, objs, meta_info in tqdm.tqdm(sampled_sequences):
        ...
        has_lex_var = meta_info['modifier'] == 'both'
        nl_loc_info = Environment.textify_location(init_s, init_loc, lex_variant=has_lex_var)  # location of boxes and agent

        observable = meta_info['observable']
        ...
        noi = meta_info['num_operations_init']
        ops_rollin = ops[:noi]
        ops_rollout = ops[noi:]
        agent_behavior = Environment.texify_agent_behavior(
            Environment.merge_ops(ops_rollin), 
